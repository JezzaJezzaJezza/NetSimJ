...
#include "Engines/CudaEngine.hpp"
#include "Traffic/AllToAll.hpp"

namespace sim {
#define SIM_USE_CUDA 1
#define SIM_LITE_ENGINE 0

  // Topology
  using BaseTopo = topo::Hypercube;
  using CSRTopo  = topo::CSRView<BaseTopo>;
  using Engine   = engines::CudaEngine<BaseTopo>;
  ...
  // Traffic
  template <typename T>
  auto gen_traffic(const T& topo) { return traffic::gen_all_to_all_traffic(topo); }
}
